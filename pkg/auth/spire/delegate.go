...
type SpireDelegateConfig struct {
	SpireAdminSocketPath string `mapstructure:"spire-admin-socket-path"`
	SpiffeTrustDomain    string `mapstructure:"spiffe-trust-domain"`
}
...
func (cfg SpireDelegateConfig) Flags(flags *pflag.FlagSet) {
	flags.StringVar(&cfg.SpireAdminSocketPath, "spire-admin-socket-path", "/run/spire/sockets/admin.sock", "The path for the SPIRE admin agent Unix socket.")
	flags.StringVar(&cfg.SpiffeTrustDomain, "spiffe-trust-domain", "spiffe.cilium.io", "The trust domain for the SPIFFE identity.")
}
