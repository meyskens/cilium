...
func (r *gatewayReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&gatewayv1alpha2.TLSRoute{},
			...
			builder.WithPredicates(onlyStatusChanged())).
		// Watch GRPC Route status changes, there is one assumption that any change in spec will
		// always update status always at least for observedGeneration value.
		Watches(&source.Kind{Type: &gatewayv1alpha2.GRPCRoute{}},
...
// enqueueRequestForOwningGRPCRoute returns an event handler for any changes with TLS Routes
// belonging to the given Gateway
func (r *gatewayReconciler) enqueueRequestForOwningGRPCRoute() handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(a client.Object) []reconcile.Request {
		hr, ok := a.(*gatewayv1alpha2.GRPCRoute)
		if !ok {
			return nil
		}
